...
LRESULT CALLBACK WndProc(HWND hwnd, UINT Message, WPARAM wParam, LPARAM lParam) {
	...
	//char str[255] = "";
	//wchar_t str[255] ;
	//LPCWSTR str = TEXT("xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx");
	//TCHAR  *edittxt[255];
	wchar_t wtext[20];
	char edittxt[255]={'\0'};
	int j = 0;
	...
	switch(Message) {
		case WM_COMMAND:
			...
			*/
			...
			switch (LOWORD(wParam)){
		        ...
		        case ID_BUTTON1:
		          	// qual botão?
		            MessageBox(hwnd, "Botao 1 foi clicado / WM_COMMAND", "Tem", MB_OK | MB_ICONINFORMATION);
					...
					//len = GetWindowText(editTotal, edittxt, len);
					//GetDlgItemText(hwnd, editTotal, edittxt, 255 );
					//edittxt[len] = 0;
	   	...
	   	case WM_PAINT:
			...
			TextOut(hdc, 15, 50, greeting, strlen(greeting));
	      	//TextOut(hdc, 15, 80, lParam, strlen(lParam));
	    ...
	    case WM_CREATE:
	        ...
	        button1 = CreateWindowEx(WS_EX_CLIENTEDGE, "BUTTON", "button1", WS_VISIBLE|WS_CHILD,
